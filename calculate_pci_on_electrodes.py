...
for count in range(0,500):
    # Take the points between -0.2s and 0.0s for each source and generate a bootstrap of them
    bs_prestimulus = np.apply_along_axis(bootstrap_resample, axis=1, arr=evoked.data[:, bootstrap_min_index:bootstrap_max_index])
...
significant_activations = evoked.data[:, post_stim_min_index:post_stim_max_index] > bootstrap_99_percentiles
...
print("Final PCI: {}".format(kolmogorov(str.encode(sig_act_sorted_string))))
